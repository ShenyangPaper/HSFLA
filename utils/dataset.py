...
class MILdataset(data.Dataset):
    def __init__(self, cfgs, split='train'):
        ...
        if split == 'train':
            self.bag_info = pd.read_csv(osp.join(self.cfgs["train_data"], 'train_2class.csv'))
            self.bag_list = osp.join(self.cfgs["train_data"], "pt_files")
        else:
            self.bag_info = pd.read_csv(osp.join(self.cfgs["test_data"], 'test_2class.csv'))   # all bag
    ...
    def df2dict(self):
        ...
        for index in range(len(self.bag_info)):
            try:
                ...
                feature = torch.load(osp.join(self.bag_list, f"{slide_id}.pt"))['features']
                patch_labels = torch.load(osp.join(self.bag_list, f"{slide_id}.pt"))['if_tumors']
                print(f'target: {target}, slide_id: {slide_id}, positive number: {torch.sum(patch_labels == 1).item()}')
                print('========================')
    ...
    def df2dict(self):
        ...
        for index in range(len(self.bag_info)):
            try:
                slide_id = self.bag_info["slide_id"].iloc[index]
                target = self.bag_info["label"].iloc[index]
                feature = torch.load(osp.join(self.bag_list, f"{slide_id}.pt"))
                bag_info[len(bag_info)] = {"slide_id": slide_id, "label": target, "num_instance": feature.size(0)}
                self.targets.append(target)
            except:
                continue
    ...
    def __getitem__(self, index):
        ...
        data = torch.load(osp.join(self.bag_list, f"{slide_id}.pt"))
        feature = data#['features']
        try:
            patch_labels = data['if_tumors']
        except:
            patch_labels = torch.zeros(feature.size()[0])
        if target == 1:
            target_ = 1
        elif target == 0:
            target_ = 0
        if self._bag_or_instance == 'bag':
            return feature, target_, patch_labels.long(), slide_id
            ...
            _patch_labels = torch.zeros(self.k)
            for i in range(len(_index)):
                _feature[i] = feature[_index[i]]
                _patch_labels[i] = patch_labels[_index[i]]
            feature = _feature
            return feature, target, _patch_labels.long(), slide_id, _index
        else:
            feature = feature[self.data_info[index]["instance_index"]]
            patch_labels = patch_labels[self.data_info[index]["instance_index"]]
            return feature, target, patch_labels.long(), slide_id
