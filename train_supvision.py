...
cfgs = load_yaml("./config/supvision.yaml")
...
def main():
    ...
    #combined_dataset = ConcatDataset([train_dset1, train_dset2])
    train_loader = torch.utils.data.DataLoader(train_dset, batch_size=cfgs["batch_size"], shuffle=True)
    ...
    trainer = trainers_fn[cfgs["task"]](model, criterion, compute_score, optimizer, train_loader, test_loader, cfgs)
    trainer.train()
